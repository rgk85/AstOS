...
def main(args):
    ...
    os.system(f"mount {args[1]} -o subvol=@,noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 /mnt")

    for mntdir in mntdirs:
        os.system(f"mkdir /mnt/{mntdir}")
        os.system(f"mount {args[1]} -o subvol={btrdirs[mntdirs.index(mntdir)]},noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 /mnt/{mntdir}")
    ...
    mntdirs_n = mntdirs
    mntdirs_n.remove("")
    os.system(f"echo 'UUID=\"{to_uuid(args[1])}\" / btrfs subvol=@,noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2,ro 0 0' > /mnt/etc/fstab")

    for mntdir in mntdirs_n:
        os.system(f"echo 'UUID=\"{to_uuid(args[1])}\" /{mntdir} btrfs subvol=@{mntdir},noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 0 0' >> /mnt/etc/fstab")
...
#    os.system(f"mount {args[1]} -o subvol=@var,noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 /mnt/.snapshots/var/var-tmp")
    os.system(f"mount {args[1]} -o subvol=@boot,noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 /mnt/.snapshots/boot/boot-tmp")
...
#    os.system("mkdir /mnt/.snapshots/etc/etc-tmp")
    os.system(f"mount {args[1]} -o subvol=@etc,noatime,discard=async,ssd,defaults,compress=zstd,space_cache=v2 /mnt/.snapshots/etc/etc-tmp")
